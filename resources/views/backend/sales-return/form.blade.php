...
<form id="salesReturnForm" action="{{ route('sales-return.save') }}" method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle" id="srItemsTable" style="min-width:860px; table-layout:fixed;">
                <colgroup>
...
(function($) {
    ...
    /* ── Customer → Against Voucher list ───────────────────────────── */
    function loadCustomerVouchers(customerId, selectedVoucherId) {
        var $sel = $('#againstVoucherSelect');
        if (!customerId) {
            $sel.html('<option value="">-- Optional --</option>');
            return;
        }
        $.get('{{ route('sales-return.customer-vouchers') }}', {
    ...
    $('#customerSelect').on('change', function() {
        loadCustomerVouchers($(this).val(), null);
    });
    ...
    function populateFromVoucher(voucherId) {
        ...
        $.get('{{ route('sales-return.voucher-items') }}', { voucher_id: voucherId, _token: '{{ csrf_token() }}' })
            .done(function(resp) {
    ...
    @if (isset($customer_id) && $customer_id)
        loadCustomerVouchers('{{ $customer_id }}', '{{ $against_voucher_id ?? '' }}');
    @endif
    ...
    window.srValidateForm = function($form) {
        ...
        if ($('#itemRows tr.item-row').length === 0) {
            showNotification('At least one item is required. Please select a voucher to return items from.', 'error');
            valid = false;
        }
