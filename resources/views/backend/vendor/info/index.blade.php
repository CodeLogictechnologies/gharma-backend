    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#confirmDelete').on('click', function() {
                if (!deleteId) return;
                $.post('{{ route('vendor.info.delete') }}', { id: deleteId, _token: '{{ csrf_token() }}' })
            ...
            $(document).on('submit', '#vendorForm', function(e) {
                ...
                if ($phone.val().trim()) {
                    ...
                    if (!phoneRegex.test($phone.val().trim())) {
                        ...
                        $phone.siblings('.invalid-feedback').text('Please enter a valid phone number.').show();
                        valid = false;
                    }
                }
                ...
                if (!isEdit) {
                    ...
                    if (!panFile) {
                        $('#pan_image').addClass('is-invalid border-danger');
                        $('#pan_image_error').text('PAN image is required.').show();
                        valid = false;
                    }

                    var regFile = document.getElementById('registration_number_image').files[0];
                    if (!regFile) {
                        $('#registration_number_image').addClass('is-invalid border-danger');
                        $('#registration_number_image_error').text('Registration image is required.').show();
                        valid = false;
                    }
                }

                // Image type & size validation (if file selected)
                var allowedTypes = ['image/jpeg', 'image/png', 'image/jpg', 'image/gif', 'image/webp'];

                var panFileCheck = document.getElementById('pan_image').files[0];
                if (panFileCheck) {
                    if (!allowedTypes.includes(panFileCheck.type)) {
                        $('#pan_image').addClass('is-invalid border-danger');
                        $('#pan_image_error').text('Only image files are allowed (jpeg, png, jpg, gif, webp).').show();
                        valid = false;
                    } else if (panFileCheck.size > 2 * 1024 * 1024) {
                        $('#pan_image').addClass('is-invalid border-danger');
                        $('#pan_image_error').text('PAN image must not exceed 2MB.').show();
                        valid = false;
                    }
                }

                var regFileCheck = document.getElementById('registration_number_image').files[0];
                if (regFileCheck) {
                    if (!allowedTypes.includes(regFileCheck.type)) {
                        $('#registration_number_image').addClass('is-invalid border-danger');
                        $('#registration_number_image_error').text('Only image files are allowed (jpeg, png, jpg, gif, webp).').show();
                        valid = false;
                    } else if (regFileCheck.size > 2 * 1024 * 1024) {
                        $('#registration_number_image').addClass('is-invalid border-danger');
                        $('#registration_number_image_error').text('Registration image must not exceed 2MB.').show();
                        valid = false;
                    }
                }
