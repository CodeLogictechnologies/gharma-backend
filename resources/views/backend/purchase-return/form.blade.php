    ...
    <div class="modal-body">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-3">
                ...
                <input type="text" name="return_date" id="return_date_np" class="form-control" autocomplete="off"
                    value="{{ $return_date ?? '' }}" data-required>
                ...
                <select name="against_voucher_id" id="againstVoucherSelect" class="form-select">
                    ...
                    @foreach ($vendorVouchers ?? [] as $voucher)
                        <option value="{{ $voucher->id }}"
                            ...
                            {{ $voucher->voucher_no }}
                            ({{ \Carbon\Carbon::parse($voucher->voucher_date)->format('Y-m-d') }})
                        </option>
                    @endforeach
                </select>
            </div>
...
<script>
    (function($) {
        ...
        function loadVendorVouchers(vendorId, selectedVoucherId) {
            ...
            $.get('{{ route('purchase-return.vendor-vouchers') }}', {
                ...
                .done(function(resp) {
                    ...
                    $.each(resp, function(i, v) {
                        var sel = (selectedVoucherId && String(selectedVoucherId) === String(v.id)) ?
                            'selected' : '';
                        var d = v.voucher_date ? v.voucher_date.substring(0, 10) : '';
                        html += '<option value="' + v.id + '" ' + sel + '>' + v.voucher_no + ' (' + d +
                            ')</option>';
                    });
                    $sel.html(html);
                })
        ...
        function populateFromVoucher(voucherId) {
            ...
            $.get('{{ route('purchase-return.voucher-items') }}', {
        ...
        $(document).on('input change', '#purchaseReturnForm .form-control, #purchaseReturnForm .form-select',
            function() {
