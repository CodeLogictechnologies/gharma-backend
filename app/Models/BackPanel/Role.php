...
class Role extends SpatieRole
{
    ...
    public static function saveData($post)
    {
        try {
            if (!empty($post['id'])) {
                ...
                // ✅ UPDATE - use Role model not DB::table
                ...
                // ✅ INSERT - use Role model create() not DB::table
                $role = Role::create([
                    'name'         => $post['name'],
                    'order_number' => $post['order_number'],
                    'status'       => $post['status'] ?? 'Y',
            ...
            // ✅ Now $role is a model object so syncPermissions() works
    ...
    public static function list($post)
    {
        try {
            ...
            $query = Role::with('permissions') // ✅ load permissions
                ->selectRaw("(SELECT count(*) FROM roles WHERE {$cond}) AS totalrecs, name, order_number, id")
                ->whereRaw($cond);
